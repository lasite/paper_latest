...
# -*- coding: utf-8 -*-
"""
iv_c_lt_patch_feasibility.py — L_T patch L_T.1 feasibility test.

Goal: verify the LF cycle survives the parameter shift designed to put us in
the L_T < L_c branch (alpha=0.04, delta=0.50, Bi_c=0.70 default).

Test point: Bi_T=0.10. If is_oscillating and 0<xi_LCST<1 → proceed to L_T.2.
"""
...
def main():
    ...
    print("=" * 70, flush=True)
    print(" L_T patch L_T.1 feasibility test", flush=True)
    ...
    if len(crossing) > 0:
        ...
        xi_LCST = 1.0  # cold-swollen everywhere
    if (phi_max > 0.5).all():
        xi_LCST = 0.0  # hot-runaway
    ...
    print(f"   theta_surf    : [{theta_surf_trough:.3f}, {theta_surf_peak:.3f}]",
          flush=True)
    ...
    np.savez(OUT / "feasibility.npz",
             is_oscillating=is_oscillating,
             J_surf_peak=J_surf_peak,
             J_surf_trough=J_surf_trough,
             theta_surf_peak=theta_surf_peak,
             theta_surf_trough=theta_surf_trough,
             xi_LCST=xi_LCST,
             L_T=L_T, L_c=L_c,
             ...
             wall_s=wall,
             verdict=verdict,
             # Also save the full trace for plotting / diagnosis
             t=t, x=x, J=J, theta=theta, phi=phi)
    print(f"\n   saved {OUT / 'feasibility.npz'}", flush=True)
